...
[![CRAN status](https://www.r-pkg.org/badges/version/refugees)](https://cran.r-project.org/package=refugees)
...
8. `flows`: Numbers of the people forced to flee during each of the years since 1962.
...
Or retrieve the development version from Github using the [`pak`](https://pak.r-lib.org/) package:
...
Here are some examples of how you can use the `refugees`` package. The `population` dataset can be used to easily get data on forced displacement or statelessness.
...
The `population` dataset can be used to get the 10 countries of origin with highest number of refugees, people in refugee-like situations including people in need of international protection in 2022.
...
We can use `ggplot2` and the [`unhcrthemes`](https://github.com/unhcr-dataviz/unhcrthemes) to visualize our data following the [UNHCR data visualization guidelines](https://dataviz.unhcr.org/general_guidance/).
...
For more examples, see [How to use UNHCR’s refugees R package](https://www.unhcr.org/refugee-statistics/insights/explainers/refugees-r-package.html).

## License

This package is released under the [Creative Commons Attribution 4.0 International Public License](https://creativecommons.org/licenses/by/4.0/).
